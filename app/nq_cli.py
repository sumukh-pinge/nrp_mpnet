...
def default_slug_from_args(a):
    ...
    return f"tau{a.tau}_b{a.beta}_c{a.cands}_{a.teacher}_lr{a.lr}_e{a.epochs}_s{a.subset}".replace(".","")

def adapter_paths(slug, inter_dir):
    base = os.path.join(inter_dir, f"W_{slug}")
    return base + ".npy", base + "_adapter.pt"

def ensure_adapter(a, device, inter_dir, embeddings, queries_emb, passage_ids,
                   nlist, bits_sq, query_ids, query_to_gt, ALPHAS, MS_INFER):
    """
    ...
    pipe_fixed, _ = load_or_build_pipeline_for_adapter(
        ...
        embeddings=embeddings, queries_emb_sample=queries_emb,
        passage_ids_sample=passage_ids, nlist=nlist, bits_sq=bits_sq,
        intermediate_dir=inter_dir, device=device, fallback_to_baseline=True,
    )
    # Train (same defaults you used)
    W_np, logs = train_W_param(
        emb_np=embeddings, que_np=queries_emb, d_out=embeddings.shape[1],
        save_path=W_path, passage_ids_sample=passage_ids,
        query_ids_sample=query_ids, query_to_gt=query_to_gt, device=device,
        epochs=a.epochs, lr=a.lr, subset=a.subset, q_batch=a.q_batch,
        tau=a.tau, beta=a.beta, gamma=1.0, grad_clip=1.0,
        pipeline_data=pipe_fixed, ALPHAS_INFER=ALPHAS,
        MS_INFER=MS_INFER, SELECT_NPROBE=a.select_nprobe, K_final=10,
    )
    ...
    if not os.path.exists(adapter_pt2):
        raise FileNotFoundError(f"Adapter training finished but not found: {adapter_pt2}")
    print(f"✅ Adapter ready: {adapter_pt2}", flush=True)

# --------------------- CLI ---------------------

def parse_args():
    p = argparse.ArgumentParser(description="Run six NQ tests (IVF/DBAM with/without adapter).")
    p.add_argument("--dataset", choices=["beir_nq","hotpotqa"], required=True)
    ...
    p.add_argument("--intermediate_root", default=None, help="If set, overrides run_root/intermediate")
    ...
    p.add_argument("--slug", default="auto", help="Adapter slug. If 'auto', derived from hparams.")
...
def main():
    ...
    # Optional cached embed file (as in your notebook)
    embed_path = os.path.join(
        data_dir,
        "sample_passage_embeddings_nq.npy" if a.dataset=="beir_nq"
        else "sample_passage_embeddings_hotpotqa.npy"
    )

    # Load / encode
    embeddings, queries_emb, passage_ids, query_ids, query_to_gt = load_all(
        corpus_file, queries_file, qrels_file, device, embed_path=embed_path
    )
    ...
    if a.mode == "ivf_fp32":
        ...
        score, row = quick_eval_for_pipeline_ivf(
            pipeline_data=pipe, trial_slug="no_adapter",
            SELECT_NPROBE=a.select_nprobe, ALPHAS_INFER=ALPHAS, MS_INFER=MS_INFER,
            K2_FIXED=a.k2_fixed, K_FINALS=K_FINALS, results_dir=results_dir,
            query_ids_sample=query_ids, query_to_gt=query_to_gt,
        )
        row_out = dict(row)
        row_out.update({"S1":"ivf","S2":"ivf","S3":"ivf",
                        "nprobe":a.select_nprobe,"K2":a.k2_fixed})
    ...
    if a.mode == "dbam_direct_baseline":
        pipe = build_pipeline_baseline(
            slug="no_adapter",
            embeddings=embeddings, queries_emb_sample=queries_emb,
            passage_ids_sample=passage_ids, bits_sq=a.bits_sq, nlist=a.nlist,
        )
        ...
        row_out = dict(row)
        row_out.update({"S1":"direct","S2":"direct","S3":"direct",
                        "nprobe":a.select_nprobe,"K2":a.k2_fixed})
        ms_tag = f"m{MS_INFER[0]}-{MS_INFER[1]}-{MS_INFER[2]}"
        out = a.out_csv or os.path.join(results_dir, f"dbam_direct_baseline_{ms_tag}.csv")
        write_single_row_csv(out, row_out)
        return
    ...
    if a.mode == "dbam_dual_baseline":
        ...
        row_out = dict(row)
        row_out.update({"S1":"dual","S2":"dual","S3":"dual",
                        "nprobe":a.select_nprobe,"K2":a.k2_fixed})
        ms_tag = f"m{MS_INFER[0]}-{MS_INFER[1]}-{MS_INFER[2]}"
        out = a.out_csv or os.path.join(results_dir, f"dbam_dual_baseline_{ms_tag}.csv")
        write_single_row_csv(out, row_out)
        return

    # 4) IVF FP32 WITH adapter (auto-trains if missing)
    if a.mode == "ivf_fp32_adapter":
        ensure_adapter(a, device, inter_dir, embeddings, queries_emb, passage_ids,
                       a.nlist, a.bits_sq, query_ids, query_to_gt, ALPHAS, MS_INFER)
        pipe_adapt, _ = load_or_build_pipeline_for_adapter(
            trial_slug=a.slug, embeddings=embeddings, queries_emb_sample=queries_emb,
            passage_ids_sample=passage_ids, nlist=a.nlist, bits_sq=a.bits_sq,
            intermediate_dir=inter_dir, device=device,
        )
        score, row = quick_eval_for_pipeline_ivf(
            pipeline_data=pipe_adapt, trial_slug=a.slug,
            SELECT_NPROBE=a.select_nprobe, ALPHAS_INFER=ALPHAS, MS_INFER=MS_INFER,
            K2_FIXED=a.k2_fixed, K_FINALS=K_FINALS, results_dir=results_dir,
            query_ids_sample=query_ids, query_to_gt=query_to_gt,
        )
    ...
    if a.mode == "dbam_direct_adapter":
        ...
        row_out.update({"S1":"direct","S2":"direct","S3":"direct",
                        "nprobe":a.select_nprobe,"K2":a.k2_fixed})
        ms_tag = f"m{MS_INFER[0]}-{MS_INFER[1]}-{MS_INFER[2]}"
        out = a.out_csv or os.path.join(results_dir, f"dbam_direct_with_adapter_{ms_tag}.csv")
        write_single_row_csv(out, row_out)
        return
    ...
    if a.mode == "dbam_dual_adapter":
        ...
        row_out.update({"S1":"dual","S2":"dual","S3":"dual",
                        "nprobe":a.select_nprobe,"K2":a.k2_fixed})
        ms_tag = f"m{MS_INFER[0]}-{MS_INFER[1]}-{MS_INFER[2]}"
        out = a.out_csv or os.path.join(results_dir, f"dbam_dual_with_adapter_{ms_tag}.csv")
        write_single_row_csv(out, row_out)
        return
